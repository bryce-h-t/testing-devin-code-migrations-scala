...
object SparkOpInstance2 extends SparkOp {
  ...
  override def inputs: Set[String] = Set(domainA.SparkOpInstance1.name) // Reference to SparkOpInstance1 as an input using fully qualified name
  override def query(inputs: Map[String, DataFrame]): DataFrame = {
    // Using randomValue from SparkOpInstance1 as instructed
    val _ = domainA.SparkOpInstance1.randomValue
